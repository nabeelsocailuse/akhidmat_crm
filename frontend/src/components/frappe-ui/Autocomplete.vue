...
// Fix the valuePropPassed logic to properly handle v-model
const valuePropPassed = computed(() => {
  // Only consider value prop passed if modelValue is NOT present
  // This ensures v-model works correctly
  return 'value' in attrs && !('modelValue' in attrs)
})
...
const selectedValue = computed({
  ...
  set(val) {
    // Fix: Clear query and close options only after ensuring the value is properly set
    if (val) {
      // Extract the value from the option object if it's an object
      const valueToEmit = typeof val === 'object' && val !== null ? val.value : val
