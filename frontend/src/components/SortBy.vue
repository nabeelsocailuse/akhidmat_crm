<template>
  ...
  >
    <template #target="{ togglePopover }">
      <AppStyling buttonLabel="Sort" @click="togglePopover()" type="button" buttonType="sort">
  ...
  <NestedPopover v-else>
    <template #target="{ open }">
      <AppStyling v-if="sortValues.size > 1" buttonLabel="Sort" type="button" buttonType="sort">
        <template v-if="hideLabel">
          <SortIcon class="h-4" />
        </template>
