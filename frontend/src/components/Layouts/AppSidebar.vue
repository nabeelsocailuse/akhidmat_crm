<template>
  ...
  >
    ...
    <div class="flex-1 overflow-y-auto px-3">
      ...
      <div class="mb-4">
        <div
          class="flex items-center gap-3 rounded-xl py-2 px-4 transition-all duration-200 hover:bg-purple-50 cursor-pointer"
          @click="() => toggleNotificationPanel()"
        >
          <div class="flex h-5 w-5 items-center justify-center text-gray-700">
            <NotificationsIcon class="h-4 w-4" />
          </div>
          <span v-if="!isSidebarCollapsed" class="text-sm font-medium text-black">
            {{ __('Notifications') }}
          </span>
          <Badge
      ...
      <!-- Main Navigation -->
      <div v-for="view in allViews" :key="view.label" class="mb-4">
        <div
          v-if="!view.hideLabel && isSidebarCollapsed && view.views?.length"
          class="mx-2 my-2 h-1 border-b border-gray-300"
        />
        <Section
          :label="view.name"
        ...
        >
          ...
          <nav class="flex flex-col gap-1">
            <SidebarLink
              ...
              class="mx-2 my-0.5 rounded-xl hover:bg-purple-50"
            />
          </nav>
        </Section>
      </div>
    ...
    <div class="p-3">
      ...
      <div v-if="!isOnboardingStepsCompleted && !isSidebarCollapsed" class="mb-4">
        <div class="rounded-xl bg-gradient-to-r from-pink-500 to-purple-600 p-4 text-white">
          <h3 class="font-bold text-sm mb-2">Getting Started</h3>
          <p class="text-xs mb-3 opacity-90">2/9 steps</p>
          <button class="w-full bg-white text-purple-600 text-xs font-medium py-2 px-3 rounded-xl flex items-center justify-center gap-2 hover:bg-gray-50 transition-colors">
            Continue
      ...
      <div class="space-y-2">
        <SidebarLink
          ...
          "
          class="rounded-xl hover:bg-purple-50"
        >
          <template #icon>
