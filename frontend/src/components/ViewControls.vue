...
function getParams() {
  ...
  const filters = (() => {
  try {
    return (_view?.filters && JSON.parse(_view.filters)) || {}
  } catch (e) {
  ...
  // Merge with props filters
  const finalFilters = { ...filters, ...props.filters }
...
async function exportRows() {
  // Add null check for list.value.data
  if (!list.value?.data?.columns) {
  ...
  let order_by = list.value.params?.order_by || 'modified desc'
  let page_length = list.value.params?.page_length || 20
  
  if (export_all.value) {
    page_length = list.value.data.total_count || 0
  }
  ...
  standardViews.push({
    ...
    onClick() {
      ...
      try {
        ...
        // Fallback navigation
        window.location.href = `${window.location.pathname}?viewType=kanban`
      }
    },
  })
}
