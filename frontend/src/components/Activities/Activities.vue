...
const all_activities = createResource({
  ...
  params: { name: doc.value?.name || props.docname },
  cache: ['activity', doc.value?.name || props.docname],
  auto: doc.value?.name ? true : false,
...
// Add a watcher to reload when document data becomes available
watch(() => doc.value?.name, (newName) => {
  if (newName && newName !== props.docname) {
    all_activities.reload()
  }
}, { immediate: true })
