...
function isPreservingUserModifications() {
  if (
    ...
    return document.doc.deduction_breakeven.some(
      (row) => row._isUserModified || row._userModifiedPercentage
    );
  }
  return false;
}
        ...
        if (deductionDetails && deductionDetails.length > 0) {
          ...
          for (const deductionDetail of deductionDetails) {
            const percentageAmount =
              paymentRow.donation_amount * (deductionDetail.percentage / 100);
...
async function populateDeductionBreakevenFromAPI() {
  ...
  // Ensure every payment row has a random_id before any downstream logic
  // This covers cases after delete->add where the new row may be missing an id
  ...
  // Check for Zakat intention presence and collect ids where available
...
async function updateDeductionBreakevenFromAPI() {
  ...
  try {
    const result = await call(
      "crm.fcrm.doctype.donation.api.update_deduction_breakeven",
      {
        payment_details: document.doc.payment_detail,
...
async function setDeductionBreakevenFromAPI() {
  ...
  try {
    ...
    // Note: Duplicate prevention and old entry clearing is now handled in the payment_detail watcher
...
function validateField(fieldName, value, rowIndex = null) {
  ...
  if (
    rowIndex !== null &&
    document.doc.payment_detail &&
    document.doc.payment_detail[rowIndex]
  ) {
...
async function submitDonation() {
  try {
    ...
    // Validate full form including deduction percentage constraints
    if (!(await validateAndShowErrors())) {
      return;
    }

    // Ensure latest in-memory changes (e.g., deduction_breakeven percentages) are persisted before submit
    ...
    if (result) {
      ...
      // Immediately reload the document so UI reflects Submitted status and updated actions
      try {
        await document.reload();
      } catch (e) {
        // Fallback to hard reload if document reload fails
        setTimeout(() => {
