  ...
  const paymentEntrySerialsByDonor = ref({});
  ...
  function openPaymentEntryModal() {
    // Build donor and serial options from payment_detail but only include those with outstanding > 0
    const rows = (document.doc && Array.isArray(document.doc.payment_detail)) ? document.doc.payment_detail : [];
    paymentEntrySerialsByDonor.value = {};
    paymentEntryDonorOptions.value = [];

    // For each payment detail row, check outstanding amount via backend and populate mapping
    const donorsSeen = new Set();
    const tasks = [];
    for (const r of rows) {
      if (!r || !r.donor) continue;
      const donorId = r.donor;
      const idx = Number(r.idx || r.serial_no || r.name || 0);
      // call get_outstanding for this donor+idx
      tasks.push(
        (async () => {
          try {
            const details = await call("akf_accounts.akf_accounts.doctype.donation.donation.get_outstanding", {
              filters: JSON.stringify({ name: document.doc.name, donor_id: donorId, idx: idx }),
            });
            const outstanding = Number(details?.outstanding_amount || 0);
            if (outstanding > 0) {
              if (!paymentEntrySerialsByDonor.value[donorId]) paymentEntrySerialsByDonor.value[donorId] = [];
              const serialLabel = String(r.idx || r.serial_no || idx || "");
              paymentEntrySerialsByDonor.value[donorId].push({ label: serialLabel, value: serialLabel });
              donorsSeen.add(donorId);
            }
          } catch (e) {
    ...
    // Wait for all lookups (small number expected)
    Promise.all(tasks).then(() => {
      // Build donor options only for donors that have outstanding serials
      paymentEntryDonorOptions.value = Array.from(Object.keys(paymentEntrySerialsByDonor.value)).map((d) => ({ label: d, value: d }));

      // Reset form and readonly metrics
  ...
  watch(() => paymentEntryForm.value.donor_id, () => {
    ...
    // Use the precomputed serials for the selected donor (only serials with outstanding > 0)
    const donorId = paymentEntryForm.value.donor_id;
    if (donorId && paymentEntrySerialsByDonor.value[donorId]) {
      paymentEntrySerialOptions.value = paymentEntrySerialsByDonor.value[donorId];
    } else {
      paymentEntrySerialOptions.value = [];
    }
  });
