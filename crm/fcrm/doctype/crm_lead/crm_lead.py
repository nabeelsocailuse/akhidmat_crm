...
class CRMLead(Document):
	...
	def create_donor(self, contact, organization, donor=None):
		new_donor = frappe.new_doc("Donor")

		lead_donor_map = {
			"lead_owner": "donor_owner",
		}
		...
		for field in self.meta.fields:
			...
			fieldname = field.fieldname
			if field.fieldname in lead_donor_map:
				fieldname = lead_donor_map[field.fieldname]

			if hasattr(new_donor, fieldname):
				if fieldname == "organization":
		...
		new_donor.update(
			{
				...
				"donor_identity": self.custom_lead_identity_,
...
@frappe.whitelist()
def convert_to_deal(lead, doc=None, donor=None, existing_contact=None, existing_organization=None):
	if not (doc and doc.flags.get("ignore_permissions")) and not frappe.has_permission(
		"CRM Lead", "write", lead
	):
		frappe.throw(_("Not allowed to convert Lead to Donor"), frappe.PermissionError)
...
# @frappe.whitelist()
# def convert_to_donor(lead, doc=None, donor=None, existing_contact=None, existing_organization=None):	
# 	if not (doc and doc.flags.get("ignore_permissions")) and not frappe.has_permission(
# 		"CRM Lead", "write", lead
# 	):
# 		frappe.throw(_("Not allowed to convert Lead to Donor"), frappe.PermissionError)
...
# 	frappe.msgprint(
# 		_("Lead '{0}' converted to Donor '{1}'").format(
