	...
	for tab in tabs:
		for section in tab.get("sections"):
			...
			for column in section.get("columns") if section.get("columns") else []:
				...
				for i, fieldname in enumerate(column.get("fields") if column.get("fields") else []):
					# FIX: fieldname is a string, not a field object
					if isinstance(fieldname, str):
						field_obj = next((f for f in fields if f.fieldname == fieldname), None)
						if field_obj:
							field_dict = field_obj.as_dict()
							handle_perm_level_restrictions(field_dict, doctype, parent_doctype)
							column["fields"][i] = field_dict
					else:
						# If it's already a field object, keep it
						...
						if (
							...
							and field_dict.reqd
							and any(f.get("fieldname") == field_dict.get("fieldname") for f in required_fields)
						):
							required_fields = [
								f for f in required_fields if f.get("fieldname") != field_dict.get("fieldname")
							]
